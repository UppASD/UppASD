...
CudaMeasurement::CudaMeasurement(
	const cudaMatrix<real,3,3> &p1,
	const cudaMatrix<real,3,3> &p2,
	const cudaMatrix<real,2>   &p3,
	bool p4, bool p5) :
		emomM(p1), emom(p2), mmom(p3), fastCopy(p4), alwaysCopy(p5),
	...
	if (fastCopy) {
		...
		pinned_emomM = pinned_emom = pinned_mmom = NULL;
		...
		if (cudaPeekAtLastError() == cudaErrorMemoryAllocation) {
			...
			if (pinned_emomM != NULL) cudaFreeHost(pinned_emomM);
			if (pinned_emom  != NULL) cudaFreeHost(pinned_emom);
			if (pinned_mmom  != NULL) cudaFreeHost(pinned_mmom);
...
void CudaMeasurement::measure(size_t mstep) {
	...
	if (copy) {
		// Copy and queue
		if (fastCopy) copyQueueFast(mstep);
		else          copyQueueSlow(mstep);
	}
	else {
