...
CudaMomentUpdater::CudaMomentUpdater(cudaMatrix<real,2> &p1, cudaMatrix<real,2> &p2,
		cudaMatrix<real,2> &p3, cudaMatrix<real,3,3> &p4, cudaMatrix<real,3,3> &p5, 
		cudaMatrix<real,3,3> &p6, cudaMatrix<real,2> &p7, int p8, char p9) : 
			mmom(p1), mmom0(p2), mmom2(p3), emom(p4), emom2(p5), emomM(p6), mmomi(p7), 
			mompar(p8), initexc(p9),
			stopwatch(GlobalStopwatchPool::get("Cuda moment")),
			parallel(CudaParallelizationHelper::def) {
	// Exit if mompar is not supported
	if (mompar == 3) {
		fprintf(stderr, "mompar 3 (ptnanowire) not implemented!\n");
		exit(EXIT_FAILURE);
	}
}
