...
bool CudaMdSimulation::initiateMatrices() {
   ...
   if(!beff.has_data() || !b2eff.has_data() || !emomM.has_data() || !emom.has_data() || !emom2.has_data()
      || !external_field.has_data() || !mmom.has_data()
      || (!btorque.has_data() && (FortranData::btorque != nullptr)) ||
      //	    !temperature   .has_data() ||
      !mmom0.has_data() || !mmom2.has_data() || !mmomi.has_data()) {
      release();
      // Check for error
...
void CudaMdSimulation::measurementPhase() {
   ...
   // Moment updater
   CudaMomentUpdater momUpdater(mmom, mmom0, mmom2, emom, emom2, emomM, mmomi, mompar, initexc);
