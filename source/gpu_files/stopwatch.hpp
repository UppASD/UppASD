...
public:
	...
	void add(const char * name)             {add(std::string(name));}
	void add(const char * name, size_t len) {add(std::string(name, len));}
	void add(const std::string &name) {
		struct timeval now;
		gettimeofday(&now, 0);
	...
	// Empty stopwatch?
	bool empty() {
		if (time_list.empty()) return true;
		if (time_list.size() == 1) {
...
private:
	inline float diff(const struct timeval * a, const struct timeval * b) {
		return  (float) (  (1000.0 * (b->tv_sec - a->tv_sec) )
		                 + ( 0.001 * (b->tv_usec - a->tv_usec) ) );
	}
