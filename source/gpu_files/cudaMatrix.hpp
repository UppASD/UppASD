...
#ifndef __CUDA_MATRIX_HPP__
#define __CUDA_MATRIX_HPP__
...
private:
	...
	bool init(size_t i, size_t j, size_t k, size_t l) {
		...
		if (e != cudaSuccess) {
			#ifdef DEBUG
			printf("cudaMatrix::initiate: cudaMalloc returned %d (%s)"
			       " when trying to allocate %ld bytes\n", 
			       e, cudaGetErrorString(e), size);
			__MAT_ERR();
			#endif
			return false;
		}
...
public:
	// Constructors
	cudaMatrix() {}
	cudaMatrix(const hostMatrix<T,D,I,J,K,L> &m) {clone(m);}
...
//	}
	...
	inline bool memcopy(const hostMatrix<T,D,I,J,K,L> &m) {
		#ifdef DEBUG
		for (int n = 0; n < D; n++) if (this->dim_size[n] != m.dimension_size(n)) {
		...
		// Copy data
		cudaError_t e = cudaMemcpy(this->data, m.get_data(), this->data_size(), 
	...
	inline bool memcopy(const cudaMatrix<T,D,I,J,K,L> &m, cudaStream_t stream = 0) {
		...
		// Copy data
		cudaError_t e = cudaMemcpyAsync(this->data, m.data, this->data_size(),
	...
	inline bool memcopyTo(hostMatrix<T,D,I,J,K,L> &m) const {
		...
		// Copy data
		cudaError_t e = cudaMemcpyAsync(m.get_data(), this->data, this->data_size(), 
