...
namespace kernels
{
    ...
    __global__ void pontryagin_no(const CudaTensor<real, 3>& emomM,
                                  const CudaTensor<real, 4>& grad_mom,
                                  uint sk_num_count,
                                  SkyrmionNumberData& d);

    __global__ void pontryagin_tri(const CudaTensor<real, 3>& emomM,
                                  const CudaTensor<real, 4>& grad_mom,
                                  const CudaTensor<real, 2>& simp,
                                  uint sk_num_count,
                                  SkyrmionNumberData& d);


    // init Delaunay simplices
