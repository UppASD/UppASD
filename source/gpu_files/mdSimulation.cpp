...
void MdSimulation::measurementPhase() {
	...
	HamiltonianCalculations ham(ncoup, nlist, nlistsize,
				    do_dm, 
				    dmvect, dmlist, dmlistsize);
		...
		if (nstep > 20) {
			if ((mstep + 1) % ((rstep+nstep)/20) == 0) {
				...
				printf("C/C++: %2ld%% done. Mbar: %10.6f. U: %8.5f.\n",
					(mstep + 1)*100/(rstep+nstep), *mavg, *binderc);
...
void MdSimulation::initiateOwn() {
	...
	nlist         .set(new unsigned int[max_no_neigh*N],max_no_neigh,N);
	nlistsize     .set(new unsigned int[N]             ,N);
...
void MdSimulation::freeOwn() {
	if (isOwnData) {
		...
		// Reset
		ncoup         .set(NULL,0,0,0);
		nlist         .set(NULL,0,0,0);
		nlistsize     .set(NULL,0,0,0);
		beff          .set(NULL,0,0,0);
		b2eff         .set(NULL,0,0,0);
		emomM         .set(NULL,0,0,0);
		emom          .set(NULL,0,0,0);
		emom2         .set(NULL,0,0,0);
		external_field.set(NULL,0,0,0);
		btorque       .set(NULL,0,0,0);
		mmom0         .set(NULL,0,0);
		mmom2         .set(NULL,0,0);
		mmomi         .set(NULL,0,0);
		mmom          .set(NULL,0,0);
		temperature   .set(NULL,0);
	}
