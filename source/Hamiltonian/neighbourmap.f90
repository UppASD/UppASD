...
contains
   ...
   subroutine setup_nm(Natom,NT,NA,N1,N2,N3,C1,C2,C3,BC1,BC2,BC3,block_size,atype,  &
      ...
      ! Keep the legacy matching radius (tol=0.01 was compared with the
      ! squared residual).  Some established inputs intentionally use a
      ! slightly distorted basis together with idealized lattice symmetry.
      real(dblprec), parameter :: map_tol  = 1.0d-1
      ...
      logical :: is_periodic, is_dilute, has_open_boundary
      logical :: typematch
      !
      ! A fully open single cell is an arbitrary finite cluster, not a
      ! periodically repeated basis.  Its neighbour vectors need not differ
      ! from a basis site by an integer lattice translation, which is the
      ! invariant used by the stable compressed-cell mapper below.  Preserve
      ! the direct finite-cluster lookup for this geometry.
      if (N1*N2*N3 == 1 .and. BC1 /= 'P' .and. BC2 /= 'P' .and. BC3 /= 'P') then
         call setup_nm_old(Natom,NT,NA,N1,N2,N3,C1,C2,C3,BC1,BC2,BC3,block_size,atype, &
            Bas,max_no_neigh,max_no_shells,max_no_equiv,sym,nn,redcoord,nm,nmdim,     &
            do_ralloy,Natom_full,acellnumb,atype_ch,nntype)
         return
      end if
      ...
      has_open_boundary = (BC1 /= 'P' .or. BC2 /= 'P' .or. BC3 /= 'P')
      !
      if (N1*N2*N3 > 1) then
      ...
      do i0 = 1, NA
         ...
         do ishell = 1, NN(itype)
            ...
            do inei = 1, nmdimt(ishell,itype)
               ...
               !
               if (nmatch == 0) then
                  ! An open boundary can legitimately remove a
                  ! symmetry-related image of a neighbour shell. Keep the
                  ! images present in the finite geometry, as setup_nm_old
                  ! did, while retaining strict validation for bulk cells.
                  if (has_open_boundary) cycle
            ...
            !
            ! A periodic shell must map completely.  With an open boundary,
            ! its symmetry-related images can be genuinely absent.
            if (counter /= nmdimt(ishell,itype) .and. .not. has_open_boundary) then
               !$omp critical(setup_nm_error_output)
               write(*,'(a,4i8)') &
