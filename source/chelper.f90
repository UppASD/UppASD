...
module Chelper
   ...
   ! Branch-compatibility buffers for GPU bridge pointers. In this branch these
   ! arrays are not exported by the legacy measurement modules.
   real(dblprec), dimension(:,:,:), allocatable, target :: mavg2_buff_bridge
   real(dblprec), dimension(:,:,:), allocatable, target :: mavg4_buff_bridge
   real(dblprec), dimension(:,:,:,:), allocatable, target :: mavg4_buff_proj_bridge
   real(dblprec), dimension(:), allocatable, target :: eavg_buff_bridge
   real(dblprec), dimension(:), allocatable, target :: eavg2_buff_bridge
   real(dblprec), dimension(:,:,:), allocatable, target :: mmomb_bridge
...
contains
   ...
   subroutine ensure_gpu_bridge_buffers()
      implicit none
      integer :: traj_nmax

      traj_nmax = 1
      if (allocated(traj_buff)) traj_nmax = max(1, maxval(traj_buff))

      if (.not.allocated(mavg2_buff_bridge)) then
         if (allocated(mavg_buff)) then
            allocate(mavg2_buff_bridge(size(mavg_buff,1), size(mavg_buff,2), size(mavg_buff,3)))
         else
            allocate(mavg2_buff_bridge(1,1,1))
         end if
         mavg2_buff_bridge = 0.0_dblprec
      end if

      if (.not.allocated(mavg4_buff_bridge)) then
         if (allocated(mavg_buff)) then
            allocate(mavg4_buff_bridge(size(mavg_buff,1), size(mavg_buff,2), size(mavg_buff,3)))
         else
            allocate(mavg4_buff_bridge(1,1,1))
         end if
         mavg4_buff_bridge = 0.0_dblprec
      end if

      if (.not.allocated(mavg4_buff_proj_bridge)) then
         if (allocated(mavg_buff_proj)) then
            allocate(mavg4_buff_proj_bridge(size(mavg_buff_proj,1), size(mavg_buff_proj,2), size(mavg_buff_proj,3), size(mavg_buff_proj,4)))
         else
            allocate(mavg4_buff_proj_bridge(1,1,1,1))
         end if
         mavg4_buff_proj_bridge = 0.0_dblprec
      end if

      if (.not.allocated(eavg_buff_bridge)) then
         allocate(eavg_buff_bridge(max(1, ene_buff)))
         eavg_buff_bridge = 0.0_dblprec
      end if

      if (.not.allocated(eavg2_buff_bridge)) then
         allocate(eavg2_buff_bridge(max(1, ene_buff)))
         eavg2_buff_bridge = 0.0_dblprec
      end if

      if (.not.allocated(mmomb_bridge)) then
         allocate(mmomb_bridge(Natom, max(1, tottraj_buff), Mensemble))
         mmomb_bridge = 0.0_dblprec
      end if

      if (.not.allocated(emomb_bridge)) then
         allocate(emomb_bridge(3, Natom, max(1, tottraj_buff), Mensemble))
         emomb_bridge = 0.0_dblprec
      end if

      if (.not.allocated(mmomb_traj_bridge)) then
         allocate(mmomb_traj_bridge(max(1, traj_nmax), max(1, ntraj), Mensemble))
         mmomb_traj_bridge = 0.0_dblprec
      end if

      if (.not.allocated(emomb_traj_bridge)) then
         allocate(emomb_traj_bridge(3, max(1, traj_nmax), max(1, ntraj), Mensemble))
         emomb_traj_bridge = 0.0_dblprec
      end if
   end subroutine ensure_gpu_bridge_buffers
   ...
   subroutine FortranData_Initiate(stt,btorque,cc, phase)
      ...
      call FortranData_setMeasurables( &
           mavg_buff, mavg2_buff_bridge, mavg4_buff_bridge, &
           mavg_buff_proj, mavg2_buff_proj, mavg4_buff_proj_bridge, &
           binderc, avrgmcum, avrgm2cum, avrgm4cum, &
           eavg_buff_bridge, eavg2_buff_bridge, &
           traj_step, traj_buff, traj_atom, &
           mmomb_bridge, mmomb_traj_bridge, emomb_bridge, emomb_traj_bridge, &
           spinwaitt, spinwait, &
