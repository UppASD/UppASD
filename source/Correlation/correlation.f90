!> Data and routines for calculate connected spin correlation function S(r,t)
!! and Fourier transforms S(q,w)
...
contains
   ...
   !> Calculate S(q) for a cell filling mesh for transform to S(r)
   ...
   !> Read q-points from file for S(r,t) -> S(q,t) transform
   ...
   !> Automatic setup of q-point mesh for calculating S(q)->S(r)
   ...
   !> Calculate S(q) for a cell filling mesh for transform to S(r) (sublattice projection)
...
!> Calculate current average magnetization by components for the connected S(q,w)
...
!> Allocate adaptivetime step for correlation
...
!> Calculate bimagnon correlation function
