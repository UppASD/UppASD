...
### - [x] M4 — Correlations: remove the `sc_max_nstep` dimension from device (base S(q,t))  (P1, L)  — **landed 2026-07-21; hybrid on-device transform bit-identical to prior device result (T=0 bccFe_sqw, all 40k sqw values, max Δ = 0)**
**Files:** `gpu_files/correlations/{correlation_types.h, correlation_kernels.{hpp,cpp}, gpuCorrelations.{hpp,cpp}}`.
**Task:** `qt(3, nq, sc_max_nstep)` stored the full time series on device only so the t→ω transform could run at flush. Implemented as **chunked streaming with an incremental on-device transform** (the "hybrid"): the `sc_max_nstep` buffer is gone but the DFT stays on the GPU.

**Hybrid, implemented for the base `sc` path:**
- Device `sc.qt` is a rolling `(3, nq, K)` chunk (`K = min(64, sc_max_nstep)`). The dynamic sampler writes slot `t_cur % K`; the slot is zeroed before reuse (FinalSum kernels use `+=`).
- When a chunk fills (and for the final partial chunk at flush): fold it into the device accumulator `qw(3, nq, nw)` with `GPUSwChunkAccum` — a small kernel, one thread per `(c,q,ω)`, summing that chunk's slices with global time index: `qw(c,q,ω) += Σ_{t∈chunk} qt(c,q,t)·exp(+i·t·dt·w(ω))·win(t+1, sc_max_nstep)` (`dt = delta_t·sc_step`, constant on the GPU path). The transform is additive over time so successive chunks accumulate; chunks are serialized on the work stream so no atomics are needed.
- The same chunk is also streamed `D2H` into the host `m_kt(3,nq,sc_max_nstep)` buffer (column-major → contiguous, single memcpy) so the `sqt` output for `do_sc T`/`Y` is preserved.
- At flush, `qw` is copied to host `m_kw`. Device `w_block` scratch is **removed** entirely; the base device footprint drops from `3·nq·(sc_max_nstep + nw + blocks·nw)` complex to `3·nq·(K + nw)` — for the usual regime (`nq~100, nw~1000`) that's a few MB, dominated by `qw`, with a full GPU transform.

**Why hybrid over host-finish:** finishing the DFT on the host would shrink the device to `3·nq·K`, but the transform is `O(nq·nw·sc_max_nstep)` with a `sin/cos` per term — far slower on CPU, and only worth it when even `qw(3,nq,nw)` will not fit (very large BZ meshes). For typical q-*paths* the hybrid keeps the fast GPU transform at negligible extra memory. **Why not option (a) as originally written:** it discarded `m_kt` (needed for `sqt`); streaming `m_kt` to host alongside the device `qw` accumulation resolves that.

**Still open:** the projected paths (`sc_proj`/`sc_projch`, `qt(3, nproj, nq, sc_max_nstep)`) still keep the full series + oversized `w_block` on device — apply the same hybrid there, plus `w_block` right-sizing from actual launch geometry.
**Acceptance:** Dynamic-correlation case (`do_gpu_correlations Y`, `do_sc Q`/`Y`) matches the pre-change device result to printed precision and Fortran `m_kw` within tolerance; tracker shows the `sc_max_nstep`-proportional base allocation and the `w_block` scratch gone.
