"""Simple plotting helpers for UppASD notebooks.

Design goals:
- Each measurable is plotted with its own time array (no interpolation/resampling).
- Provide optional conversion to physical time units via `timestep` (seconds).
- Keep API small: `to_physical_time()` and `plot_series()`.
"""
