...
#
# Dependencies: python>3.0, numpy, spglib, seekpath, tabulate. All
...
def Run(cell, simid, mesh, posfiletype):
    ...
    for row in range(mypath.shape[0] - 1):
        ...
        for iint in range(nsteps):
            ...
            xpath = np.append(
                xpath,
                mypath[row, :] * (1.0 - ipart) + mypath[row + 1, :] * ipart,
                axis=0,
            )
            if iint < nsteps - 1:
                lpath.append(" ")
    lpath.append(
        [
            k
            for k, v in kpath_obj["point_coords"].items()
            if (v == mypath[mypath.shape[0] - 1]).all()
        ][0]
    )
