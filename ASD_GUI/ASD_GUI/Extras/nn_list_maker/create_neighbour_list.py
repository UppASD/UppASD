...
def get_symmetry_ops_from_dataset(sym_data):
    ...
    symop_list = [
        (r, t)
        for r, t in zip(sym_data["cart_rotations"], sym_data["cart_translations"])
    ]
...
def get_symmetry_data_from_structure(cell):
    ...
    # %%
    # Augment spglib dataset with transformed rotations which work on
    # primitive cells
    ...
    for rotation, translation in zip(sym_data["rotations"], sym_data["translations"]):
        ...
        sym_data["cart_translations"].append(np.matmul(lattice.T, translation))
