...
class ASDMomActors:
    ...
    def Add_MomActors(self, ren, renWin, iren, ASDdata, window):
        ...
        ASDMomActors.height = (
            max(ASDMomActors.xmax, ASDMomActors.ymax, ASDMomActors.zmax) * 1.75
        )
        ...
        ASDMomActors.ungrid.GetPointData().SetScalars(
            ASDMomActors.src.GetPointData().GetScalars()
        )
        ...
        if ASDdata.flag2D:
            ...
            ASDMomActors.MagDensMap.SetInputConnection(
                ASDMomActors.MagDensMethod.GetOutputPort()
            )
            ...
            # decreases rapidly
            # -------------------------------------------------------------------
            dist = np.asarray(ASDMomActors.src.GetPoint(0)) - np.asarray(
                ASDMomActors.src.GetPoint(1)
            )
            ...
            # -------------------------------------------------------------------
            ASDMomActors.MagDensMethod.SetModelBounds(
                ASDMomActors.xmin,
                ASDMomActors.xmax,
                ASDMomActors.ymin,
                ASDMomActors.ymax,
            ...
            if (
                ...
                ASDMomActors.MagDensMethod.SetSampleDimensions(
                    3, int(ASDMomActors.ymax), int(ASDMomActors.zmax)
                )
            elif (
                ...
                ASDMomActors.MagDensMethod.SetSampleDimensions(
                    int(ASDMomActors.xmax), 3, int(ASDMomActors.zmax)
                )
            elif (
                ...
                ASDMomActors.MagDensMethod.SetSampleDimensions(
                    int(ASDMomActors.xmax), int(ASDMomActors.ymax), 3
                )
        ...
        ASDMomActors.spinarrownormals.SetInputConnection(
            ASDMomActors.spinarrowtriangles.GetOutputPort()
        )

        ASDMomActors.spinarrowtcoords = vtk.vtkTextureMapToCylinder()
        ASDMomActors.spinarrowtcoords.SetInputConnection(
            ASDMomActors.spinarrownormals.GetOutputPort()
        )
        ASDMomActors.spinarrowtcoords.PreventSeamOn()

        ASDMomActors.spinarrowtangents = vtk.vtkPolyDataTangents()
        ASDMomActors.spinarrowtangents.SetInputConnection(
            ASDMomActors.spinarrowtcoords.GetOutputPort()
        )

        # Create the mapper for the spins
        ASDMomActors.SpinMapper = vtk.vtkGlyph3DMapper()
        ASDMomActors.SpinMapper.SetSourceConnection(
            ASDMomActors.spinarrowtangents.GetOutputPort()
        )
        ...
        ASDMomActors.AtomMapper.SetSourceConnection(
            ASDMomActors.AtomSphere.GetOutputPort()
        )
