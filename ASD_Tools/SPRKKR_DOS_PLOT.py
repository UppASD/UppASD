class SPRKKRPlotDos:
    ...
    def ry_to_ev(self):
        ...
        return constants.value("Rydberg constant times hc in eV")
    ...
    def get_args(self):
        ...
        parser.add_argument(
            "--extras", "-x", help=_msg, default="DOS_inp.yml", metavar="FILE", type=str
        )
        ...
        parser.add_argument(
            "--log", "-l", help=_msg, default="cell_file.log", metavar="FILE", type=str
        )
        # error file argument
        ...
        parser.add_argument(
            "--verbose", "-v", help=_msg, default=False, action="store_true"
        )
        # Label for the full plot
        _msg = "Adds a label for the full plot."
        parser.add_argument("--flabel", "-fl", help=_msg, default=None, type=str)
        # Label for the full plot
        _msg = "Adds a label for the types plot."
        parser.add_argument("--tlabel", "-tl", help=_msg, default=None, type=str)
    ...
    def get_extras(self):
        ...
        try:
            ...
            sys.exit("Problem reading " + self.args.extras + " check the format")
        return
    ...
    def SPRKKR_DOS_parse_data(self):
            ...
            if len(line) > 0:
                if line[0] != " ":
                    ...
                    curr_data = [
                        ...
                        for ii in range(0, len(line) - 1, self.SPRKKR_char_len)
                    ]
                else:
                    tmp = self.SPRKKR_char_len * str(0) + line[self.SPRKKR_char_len :]
                    tmp = tmp.replace(" ", "")
                    curr_data = [
                        float(tmp[ii : ii + self.SPRKKR_char_len])
                        for ii in range(0, len(tmp) - 1, self.SPRKKR_char_len)
                    ]
                    count = count + 1
        ...
        for ene, nt, spin, lmn in itertools.product(
            range(0, self.num_ene),
    ...
    def get_num_colors(self):
        ...
        """
        return len([x for x in self.element_bare if x not in self.exclude_list])
    ...
    def get_DOS_max_type(self, ene_indx=-1):
        ...
        return [
            np.maximum(
                self.get_atom_DOS()[ene_indx, nt, self.lmax, 0].max(),
                self.get_atom_DOS()[ene_indx, nt, self.lmax, 1].max(),
            )
    ...
    def SPRKKR_DOS_plot(self):
        ...
        ene_indx = np.where(
            (self.get_energies() >= self.ene_range[0])
            & (self.get_energies() < self.ene_range[1])
        )[0]
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=self.fig_size)
        #######################################################################
        ...
        ax1.set_xlim(
            [self.get_energies()[ene_indx].min(), self.get_energies()[ene_indx].max()]
        )
        ax1.set_ylabel(y_label[0], fontsize=self.fig_font)
        ax1.axvline(0, color="black", linestyle="--", linewidth=1.25)
        ax1.tick_params(axis="x", colors="black", labelsize=self.ax_font, width=0)
        ax1.tick_params(axis="y", colors="black", labelsize=self.ax_font, width=2)
        ...
        ax2.tick_params(axis="x", colors="black", labelsize=self.ax_font, width=2)
        ax2.tick_params(axis="y", colors="black", labelsize=self.ax_font, width=2)
        ...
        fig.savefig(fig_name, transparent=False, dpi=self.fig_dpi, bbox_inches="tight")
        plt.clf()
        #######################################################################
        ...
        for nt in range(0, self.num_types):
            if self.element_bare[nt] not in self.exclude_list:
                ...
                for lnm in range(0, self.lmax):
                    loc_max_site[lnm] = np.maximum(
                        self.get_atom_DOS()[ene_indx, nt, lnm, 0].max(),
                        self.get_atom_DOS()[ene_indx, nt, lnm, 1].max(),
                    )
                ...
                ax1.set_xlim(
                    [
                        self.get_energies()[ene_indx].min(),
                ...
                ax1.axvline(0, color="black", linestyle="--", linewidth=1.25)
                ax1.tick_params(
                    axis="x", colors="black", labelsize=self.ax_font, width=0
                )
                ax1.tick_params(
                    axis="y", colors="black", labelsize=self.ax_font, width=2
                )
                for axis in ["top", "left", "right"]:
                    ax1.spines[axis].set_linewidth(3)
                for axis in ["bottom"]:
                ...
                ax2.axvline(0, color="black", linestyle="--", linewidth=1.25)
                ax2.tick_params(
                    axis="x", colors="black", labelsize=self.ax_font, width=2
                )
                ax2.tick_params(
                    axis="y", colors="black", labelsize=self.ax_font, width=2
                )
                ...
                fig_name = (
                    ...
                    + self.sys_name
                    + "_"
                    + self.get_atom_types()[nt][1:_nt]
                    + ".pdf"
                )
                # Save the figure to file
                fig.savefig(
                    fig_name, transparent=False, dpi=self.fig_dpi, bbox_inches="tight"
                )
                plt.clf()
        return
