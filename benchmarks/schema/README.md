...
| `campaign_manifest.v1.schema.json` | — (WP-09) | A declarative campaign: which cases/variants/sizes, which backends/precisions, sample count, measurement profile and environment-quality mode, at LEAN or FULL tier. Loaded by `benchmarks/harness/campaigns.py`, not by `schema_validate.py` — it describes intent, not a result record, and never bakes in a concrete OpenMP thread-count list (that is resolved against real host topology only once bound to a machine, mirroring `omp_sweep.py`). |
